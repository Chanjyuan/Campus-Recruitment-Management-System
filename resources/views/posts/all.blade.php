        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header bg-primary">
                    <h4 class="page-title text-truncate text-white font-weight-medium mb-0">All Posted Jobs</h4>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="multi_col_order"
                            ...
                            <tbody>
                              ...
                              <tr>
                                <td>{{$no}}</td>
                                <td><h5 class="mb-0"><a href="/posts/{{$post->id}}">{{$post->title}}</a></h5>
                                    <span class="font-14">{{$post->jobtype->name}}</span>
                                    @if($post->status == 'Available')
                                    <span class="badge badge-primary">Active</span>
                                    @else
                                    <span class="badge badge-secondary">Inactive</span>
                                    @endif</td>
                                <td><a href="/company-profile/{{$post->employer->id}}">{{$post->employer->company_name}}</a></td>
                                ...
                                <td class="btn-toolbar justify-content-center">
                                    @if($post->status == 'Available')
                                    {!! Form::open(['action' => ['AdminController@deactivate', $post->id], 'method' => 'POST']) !!}
                                    ...
                                    {!! Form::close() !!}
                                    {!! Form::open(['action' => ['AdminController@delete', $post->id], 'method' => 'POST']) !!}
                                        {{Form::hidden('_method', 'POST')}}
                                    ...
                                    {!! Form::close() !!}
                                    @else
                                    {!! Form::open(['action' => ['AdminController@activate', $post->id], 'method' => 'POST']) !!}
                                        {{Form::hidden('_method', 'POST')}}
